...
class LinkServer:
    ...
    def __init__(
        ...
        *,
        ...
        self.last_q_cmd: Optional[proto.SimQ] = None
    ...
    def _read_hw_state(self) -> None:
        ...
        try:
            with self._hw_lock:
                currents_by_id = self.hw.get_present_currents_ma()
        except Exception:
            currents_by_id = {}
        ...
        self.last_currents = {
            "linear": currents_by_id.get(self._ids[0]),
            "roll": currents_by_id.get(self._ids[1]),
            "s1": currents_by_id.get(self._ids[2]),
            "s2": currents_by_id.get(self._ids[3]),
        }
            ...
            if (now - self._t_state) >= self._state_period:
                ...
                self._broadcast(
                    proto.pack_state(
                        u=self.last_u,
                        q=self.last_q,
                        q_cmd=self.last_q_cmd,
                        currents=self.last_currents,
