...
def pack_state(
    *,
    ...
    q_cmd: Optional[SimQ] = None,
    currents: Optional[Dict[str, Optional[float]]] = None,
    ...
    if q_cmd is not None:
        out["q_cmd"] = {"linear_m": q_cmd.linear_m, "roll_rad": q_cmd.roll_rad, "theta1_rad": q_cmd.theta1_rad, "theta2_rad": q_cmd.theta2_rad}
    if currents is not None:
        out["currents"] = {
            "linear": currents.get("linear"),
            "roll": currents.get("roll"),
            "s1": currents.get("s1"),
            "s2": currents.get("s2"),
        }
...
class LinkClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.last_q_cmd: SimQ | None = None
        self.last_currents: Dict[str, Optional[float]] = {}
    ...
    def poll(self) -> None:
        ...
        if t == "state":
            ...
            if "q_cmd" in msg:
                try:
                    self.last_q_cmd = unpack_q(msg["q_cmd"])
                except Exception:
                    pass
            if "u" in msg:
                try:
                    ...
                    pass
            if "currents" in msg and isinstance(msg.get("currents"), dict):
                raw_currents = msg.get("currents", {}) or {}
                self.last_currents = {
                    "linear": raw_currents.get("linear"),
                    "roll": raw_currents.get("roll"),
                    "s1": raw_currents.get("s1"),
                    "s2": raw_currents.get("s2"),
                }
            if "torque_enabled" in msg:
                self.torque_enabled = bool(msg.get("torque_enabled", False))
            self.is_connected = True
