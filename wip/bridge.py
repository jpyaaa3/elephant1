...
class LinkServer:
    ...
    def _handle_msg(self, ident: bytes, msg: Dict[str, Any]) -> None:
        ...
        if t == "torque_off":
            ...
            return
        if t == "ports":
            self._reply(ident, {"t": "ack", "ts": proto.now_s(), "ok": True, "device": self.device, "ports": self._list_ports(), "reason": "ports", "torque_enabled": self.torque_enabled})
            return
        if t == "set_device":
...
def run_broker_host(
    *,
    ...
    try:
        ...
        print(f"[bridge] bind={bind_addr} device={device} init_hw={bool(init_hw)}")
        server.loop_forever()
    finally:
        try:
            if hw is not None:
...
def main() -> None:
    ...
    run_broker_host(
        ...
        bind_addr=str(bundle.sim_config.zmq_endpoint),
        device="",
        init_hw=False,
