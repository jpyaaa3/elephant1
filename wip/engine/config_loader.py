...
class SimConfig:
    ...
    bridge_to_sim_endpoint: str = "tcp://127.0.0.1:5556"
    controller_to_sim_endpoint: str = "tcp://127.0.0.1:5557"
...
def _default_app_config_bundle() -> AppConfigBundle:
    return AppConfigBundle(
        ...
        sim_config=SimConfig(),
        hardware_config=HardwareConfig(),
        joint_limit=JointLimit(linear_min=-0.23, linear_max=0.01, roll_min_deg=-90.0, roll_max_deg=90.0, bend_deg=36.0),
        model_config=AppModelConfig(),
        urdf_export_config=UrdfExportConfig(),
        ik_config=IkConfig(),
        mapping_config=proto.SimMappingConfig(),
...
def _load_sim_config(cp: configparser.ConfigParser, defaults: AppConfigBundle, *, config_dir: str) -> SimConfig:
    ...
    return SimConfig(
        ...
        bridge_to_sim_endpoint=cp.get("runtime", "bridge_to_sim_endpoint", fallback=sc0.bridge_to_sim_endpoint),
        controller_to_sim_endpoint=cp.get("runtime", "controller_to_sim_endpoint", fallback=sc0.controller_to_sim_endpoint),
    )
...
def _load_model_config(cp: configparser.ConfigParser, defaults: AppConfigBundle) -> AppModelConfig:
...
def _build_mapping_config(joint_limit: JointLimit, hardware_config: HardwareConfig) -> proto.SimMappingConfig:
    return proto.SimMappingConfig(
        linear_q_min_m=joint_limit.linear_min,
        linear_q_max_m=joint_limit.linear_max,
...
def load_app_config_from_ini(path: str) -> AppConfigBundle:
    ...
    model_config_cfg = _load_model_config(cp, defaults)
