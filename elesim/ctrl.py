...
class ControlClient:
    ...
    def maybe_send_target_q(self, q: SimQ, *, source: str = "sim") -> None:
        self._maybe_send_target_q(q, source=source, target_xyz=None)
...
class ControlService:
    ...
    def send_current_target(self, *, source: str) -> None:
        if self.client is not None and (not self.state.paused):
...
class ControlPanel:
    ...
    def _draw_controls_window(self) -> None:
        if not self._ctrl_window_init:
            cond = getattr(imgui, "ONCE", getattr(imgui, "FIRST_USE_EVER", 1))
            imgui.set_next_window_size(800, 600, cond)
        ...
        if self._use_hardware and self.service.has_client():
            imgui.separator()
            imgui.text("Hardware")
            state = link_state if link_state is not None else self.service.current_host_state()
        ...
        _, paused = imgui.checkbox("pause control updates", self.state.paused)
    ...
    def run(self) -> None:
        ...
        window = glfw.create_window(800, 600, "Snake Controls (ImGui)", None, None)
        if not window:
            glfw.terminate()
...
def main() -> None:
    ...
    bundle, ik_context = ik.load_ik_context(args.config)
